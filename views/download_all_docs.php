...
// ─── GET Params ───────────────────────────────────────────────
$encrypted_v_id = $_GET['v_id'] ?? '';
$encrypted_q_id = $_GET['q_id'] ?? '';

$v_id = decrypt_id($encrypted_v_id);
$q_id = decrypt_id($encrypted_q_id);
...
// ─── Master Data ─────────────────────────────────────────────
$sql = "SELECT q.*, s.school_name, s.school_logo, s.semis_code, s.school_code, s.acronym, s.demand_no, h.code_no, s.district,
               v.vendor_name, v.vendor_letterhead, v.vendor_logo, h.head_name, r.req_date,
        ...
        LEFT JOIN tbl_outward_no o ON r.id = o.requisition_id 
        WHERE q.id = '$q_id'
        ORDER BY v.id DESC";
...
$f_year_res = mysqli_query($conn, "SELECT tenure FROM tbl_budget b JOIN tbl_tenure t ON t.id = b.tenure_id 
    WHERE b.school_id = '{$data['school_id']}' AND b.status = 'Active' LIMIT 1");
...
// Replace your existing vendor query with this:
$vendors_sql = "SELECT DISTINCT v.id, v.vendor_name, v.vendor_letterhead, v.vendor_logo 
                FROM tbl_quotation_details qd
                JOIN tbl_vendor v ON qd.vendor_id = v.id
                WHERE qd.quotation_id = '$q_id'";
...
$pdf->Ln(5);
// Agar aaj Sunday (7) hai toh +1 day karke Monday kar do
...
$pdf->Ln(10);
$pdf->Cell(180, 10, "SUB: SANCTION ORDER FOR EXPENDITURE DURING C.F.Y. $tenure_name", 0, 1, 'C');
...
// PAGE: SUPPLY ORDER
// ════════════════════════════════════════════════════════════
// --- 1. Letterhead Template Load aur Page Add ---
$fixed_school_lh = "../uploads/school_letterhead/letterhead school.pdf";
...
// Position ko right side par set karne ke liye
$pdf->SetFont('Times', 'B', 11);
...
// ════════════════════════════════════════════════════════════
// LOOP: HAR VENDOR KE LIYE ALAG QUOTATION PAGE (WITH UNIQUE DATES)
// ════════════════════════════════════════════════════════════
$vendor_index = 0;
...
while ($item = mysqli_fetch_assoc($items_res_q)) {
    ...
    // --- Vendor Logo (Randomized for Quotation Loop) ---
...
$pdf->Output('I', 'Full_Package_' . $q_id . '.pdf');
exit();
