...
if (isset($_GET['action']) && $_GET['action'] == 'auto_generate') {
    ...
    while ($head = mysqli_fetch_assoc($budget_q)) {
        ...
        WHERE iht.head_type_id = $head_id AND i.status = 'Active' AND i.item_category = 'Primary' LIMIT 1");
                ...
                if ($item = mysqli_fetch_assoc($item_res)) {
                    $p_price = $item['min_price'];
                    $p_qty = floor($allowed_limit / $p_price);
                    ...
                    if ($p_qty > 0) {
                        ...
                        $current_req_amount = $p_qty * $p_price;
                    }

                    // --- SMART LAST-ENTRY LOGIC ---
                    $remaining_gap = $allowed_limit - $current_req_amount;

                    // CHECK: Agar bacha hua total budget process_limit (200,000) se kam hai, 
                    // toh iska matlab ye last entry hai. Tab secondary items add karo.
                    // $total_remaining_in_head = $remaining_for_this_head - $spent_so_far - $current_req_amount;

                    if ($allowed_limit < 200000 && $remaining_gap >= 10) {

                        $sec_res = mysqli_query($conn, "SELECT i.id, i.min_price, i.max_price FROM tbl_item i 
                JOIN tbl_item_head_type iht ON i.id = iht.item_id 
                WHERE iht.head_type_id = $head_id 
                AND i.status = 'Active' 
                AND i.item_category = 'Secondary' 
                AND i.min_price <= $remaining_gap 
                ORDER BY RAND() LIMIT 10");

                        while ($sec_item = mysqli_fetch_assoc($sec_res)) {
                            $s_price = $sec_item['min_price'];
                            $s_qty = floor($remaining_gap / $s_price);
                            ...
                            if ($s_qty > 0) {
                                ...
                                $items_to_add[] = [
                                    'id' => $sec_item['id'],
                                    'qty' => $s_qty,
                                    'min' => $sec_item['min_price'],
                                    'max' => $sec_item['max_price']
                                ];

                                $current_req_amount += ($s_qty * $s_price);
                                $remaining_gap = $allowed_limit - $current_req_amount;
                            }
